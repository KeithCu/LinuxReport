...
SHARED_VENV_DIR = os.path.join(BASE_DIR, SHARED_VENV_APP, "venv")
SHARED_VENV_BIN = os.path.join(SHARED_VENV_DIR, "bin")
GUNICORN_WORKERS = 1
GUNICORN_THREADS = 2
...
def validate_shared_venv():
    ...
    if not os.path.exists(SHARED_VENV_BIN):
...
def generate_systemd_config(app_name, app_dir):
    ...
    gunicorn_config = os.path.join(app_dir, "gunicorn.conf.py")
...
Examples:
  ...
  sudo python3 setup_gunicorn_multi_app.py --skip-apache
    ...
    for app_name, url in apps_to_deploy.items():
        ...
        socket_path = os.path.join(SOCKET_DIR, f"{app_name}.sock")
        
        # Generate gunicorn.conf.py
        gunicorn_config_path = os.path.join(app_dir, "gunicorn.conf.py")
        ...
        systemd_config_path = os.path.join(SYSTEMD_CONF_DIR, f"gunicorn-{app_name}")
        ...
        if not args.skip_apache:
            apache_config_path = os.path.join(APACHE_CONF_DIR, f"{app_name}.conf")
